...
public class ReposeController {
    ...
    // 1. 내 Repose 조회
    @Operation(summary = "메인 페이지2 - 내 휴식 조회", description = """
    ...
    // 2. Repose 추가
    @Operation(summary = "알림 추가 페이지 - 휴식 추가", description = """
    ...
    // 3. 타이머 정지
    @Operation(summary = "실제 실행 페이지 - 휴식 정지", description = """
            휴식 정지 API입니다. 휴식 ID와 정지한 시점에 남은 휴식 시간을 입력받아 휴식을 정지합니다.\n
            해당 API는 휴식을 정지한 시점에 호출하시면 됩니다.\n
            예를 들어 30분짜리 휴식을 10분 후에 정지하면, 20분이 남은 상태로 정지됩니다. 이 경우 20분 초(seconds) 단위로 입력되어야 합니다. -> 1200 seconds
            """)
    ...
    // 4. 타이머 재개
    @Operation(summary = "실제 실행 페이지 - 휴식 시작", description = "휴식 시작 API입니다. 휴식 ID를 입력받아 휴식을 시작합니다."
            + "남은 휴식 시간을 반환합니다.")
    ...
    // 5. 오늘 하루의 결과 기록
    @Operation(summary = "오늘 하루 결과 페이지 - 휴식 완료 및 오늘의 휴식 결과 기록", description = """
    ...
    // 6. 캘린더 조회
    @Operation(summary = "달력 페이지 - 캘린더 조회", description = """
    ...
    // 7. 세부 기록 조회
    @Operation(summary = "세부 기록 페이지 - 세부 기록 조회", description = """
